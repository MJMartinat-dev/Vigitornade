...
// ═══ FETCH IGN — CONTOURS RÉGIONS (cascade multi-sources) ═══
...
export async function fetchIGNRegions() {
  // Source 1 : geo.api.gouv.fr (officielle IGN)
  try {
    const r = await fetch("https://geo.api.gouv.fr/regions?fields=nom,code,contour", { signal: AbortSignal.timeout(8000) });
    if (!r.ok) throw new Error("HTTP " + r.status);
    const data = await r.json();
    const regs = data
      .filter(d => d.contour && parseInt(d.code) >= 11 && parseInt(d.code) <= 94)
      .map(d => ({
        nm: normalizeRegionName(d.nom),
        cd: d.code,
        rc: d.code,
    ...
    if (regs.length >= 10) return { shapes: regs, src: "IGN officiel · geo.api.gouv.fr" };
    throw new Error("Données IGN insuffisantes");
  } catch (e) {
    console.warn("Source 1 (geo.api.gouv.fr) échec :", e.message);
  }

  // Source 2 : GitHub raw — france-geojson de Grégoire David (haute résolution)
  try {
    const r = await fetch("https://raw.githubusercontent.com/gregoiredavid/france-geojson/master/regions.geojson", { signal: AbortSignal.timeout(8000) });
    if (!r.ok) throw new Error("HTTP " + r.status);
    const geojson = await r.json();
    const regs = geojson.features
      .filter(f => f.properties?.code && parseInt(f.properties.code) >= 11 && parseInt(f.properties.code) <= 94)
      .map(f => ({
        nm: normalizeRegionName(f.properties.nom),
        cd: f.properties.code,
        rc: f.properties.code,
        p: geoToSvgPath(f.geometry),
        cx: geoCentroid(f.geometry),
      }))
      .filter(d => d.p.length > 10);
    if (regs.length >= 10) return { shapes: regs, src: "france-geojson · GitHub CDN" };
    throw new Error("Données GeoJSON insuffisantes");
  } catch (e) {
    console.warn("Source 2 (france-geojson) échec :", e.message);
  }

  // Source 3 : fallback intégré
  return { shapes: buildFallback(), src: "Contours intégrés (sources externes indisponibles)" };
}
