...
export default function App() {
  ...
  const stations = useMemo(() => meteo.map(s => {
    const risk = calculateSTP(s.hr, hourIdx, s.cl);
              ...
              <div style={{
                ...
                <svg viewBox={`0 0 ${MAP_W} ${MAP_H}`} style={{ width: "100%", display: "block", background: "#070b14" }}>
                  ...
                  {stations.map(s => {
                    ...
                    // Icône tornade
                    const bounce = Math.sin(tick * .6) * 2;
                    const tornado = idx >= 20 ? (
                      <text x={cx} y={cy - 18 + bounce} textAnchor="middle"
                        fontSize={idx >= 45 ? 20 : idx >= 25 ? 16 : 13} pointerEvents="none">🌪️</text>
                    ) : null;
